...
.libPaths('/gpfs/ts0/home/vv233/R/x86_64-pc-linux-gnu-library/3.5') # location of R packages


# list.of.packages <- c("data.table", "Rcpp","zoo", "bitops", "matlab", "signal", "tuneR", "mmap", "GENEAread")
# new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
# if(length(new.packages)) install.packages(new.packages)


location_configfile = "/gpfs/mrc0/projects/Research_Project-MRC158833/vv233/whitehall_pbs/config.txt" # location of config file
...
# What part of the analysis needs to be done (options: 1,2,3,4 and 5)
if (mode == 0) {
  do.report = c(2,4,5)
  mode = c()
} else {
...
config = read.csv(location_configfile,row.names = 1,stringsAsFactors = FALSE,sep = ",",strip.white = TRUE)
...
g.shell.GGIR(#=======================================
             ...
             boutdur.in = c(1,10,30,60), # duration of bouts to be calculated
